//! OpenAI API provider
//!
//! Implements [`LlmProvider`] for OpenAI's chat completions API (v1/chat/completions).
//! Compatible with any OpenAI-compatible endpoint (e.g., Azure OpenAI, local vLLM).
...
impl OpenAiProvider {
    ...
    /// Convert our Message enum to OpenAI API message format
    ...
    /// Convert our ToolSchema to OpenAI function calling format
    ...
    /// Extract token usage from the API response body
...
impl LlmProvider for OpenAiProvider {
    async fn chat(
        ...
        debug!(model = %self.model, messages = messages.len(), tools = tools.len(), "OpenAI chat request");
    ...
    async fn chat_stream(
        ...
        debug!(model = %self.model, "OpenAI streaming chat request");
        ...
        if !status.is_success() {
            ...
            // If handle_error_status returned None (shouldn't happen), still error
        ...
        // Spawn a task to read bytes and send parsed chunks via channel
    ...
    async fn chat_with_options(
        ...
        debug!(model = %self.model, "OpenAI chat_with_options request");
    ...
    async fn chat_with_usage(
        ...
        debug!(model = %self.model, "OpenAI chat_with_usage request");
...
mod tests {
    ...
    fn test_parse_text_response() {
        ...
        let result = OpenAiProvider::parse_response(&body).unwrap();
        match result {
