...
[![Crates](https://img.shields.io/badge/Workspace-20_crates-green.svg)](#架构)
[![Tests](https://img.shields.io/badge/Tests-388-brightgreen.svg)](#测试)
...
*Atta* 是切叶蚁的属名——这个物种的单个群落包含 **800 万个体**，已繁荣了 **5000 万年**。没有中央指挥调度行为，智能从简单规则、角色分工和共享通信中涌现。AttaOS 将同样的原则带入软件：众多自主 Agent 在清晰规则下协调运作，产生单个 Agent 无法独立完成的成果。

---

## 什么是 AttaOS？

AttaOS 是一个 **Rust 原生的 AI Agent 操作系统**。它将每个 Agent 视为受管进程，提供调度、隔离、安全执行和全程审计——传统操作系统为程序提供的同等保障，专为自主 AI 设计。

系统分为四层：

```
                    ┌──────────────────────────────────────────┐
  客户端             │  WebUI    Console    CLI    系统托盘      │
                    └──────────────┬───────────────────────────┘
                                   │ HTTP + WebSocket
                    ┌──────────────▼───────────────────────────┐
...
- **客户端** —— WebUI (Vue 3)、Tauri 控制台、CLI、系统托盘提供多种入口。
...
AttaOS 从 **同一套代码** 编译出两个版本。四个抽象 trait 定义了业务逻辑与基础设施之间的边界：
...
```bash
cargo build --features desktop      # 零外部依赖
cargo build --features enterprise   # 生产级基础设施
```
    ...
    └── atta-agent ────── LLM 提供者 + ReAct 引擎 + 提示系统
            ...
            └── atta-channel ─── Channel trait + 22 平台实现
                    ...
                    └── atta-core ──── 控制面（API + FlowEngine + Coordinator）
                            ...
                            ├── atta-cli ──────────── attaos 二进制
                            ├── atta-tray ─────────── 系统托盘（共享逻辑）
                            ├── atta-tray-standalone ─ 企业版托盘二进制
                            ├── atta-console ──────── Tauri 管理控制台
                            └── atta-updater ──────── Tauri 自动更新器
```
...
`atta-secret` 提供 AES-256-GCM 加密键值存储。API Key、Token、凭证永不明文存储。安全管道中的密钥脱敏确保敏感值从 Agent 输出和日志中被剥离。
...
Gate 是一等公民。Flow 可以在任意步骤要求人工签字确认，超时行为可配置（阻塞、自动批准、自动拒绝）。这使 AttaOS 适用于需要监管 AI 操作的合规环境。

### 完整审计链路

Enterprise 模式下，`AuditStore` 记录每个重要事件：任务创建、Agent 操作、工具调用、审批决策、E-Stop 激活。审计链路仅追加写入，为受监管行业提供合规记录。

### 分布式事件总线

企业部署使用 **NATS JetStream** 进行节点间通信。事件持久化、有序、可跨多个 AttaOS 实例投递。Desktop 模式使用 tokio broadcast channel 实现零依赖运行——同样的事件语义，不同的传输层。
...
| 组件 | 技术 |
|------|------|
| **Web UI** | Vue 3 + Vite + Pinia，通过 `rust-embed` 嵌入二进制 |
| **系统托盘** | `tray-icon` + `muda` 菜单集成 |
| **管理控制台** | Tauri v2 WebView（< 10 MB，对比 Electron 100+ MB） |
| **自动更新** | Tauri v2 更新器，集成 GitHub Releases |
...
```bash
attaos run [--mode desktop|enterprise] [--port 3000]  # 启动服务
attaos launch [--port 3000]                            # 带更新检查启动
attaos chat [--channel terminal]                       # 交互式聊天
attaos channels                                        # 列出可用通道
attaos task list|create|get                             # 任务管理
attaos skill list|get|run                               # Skill 管理
```
...
| 系统托盘 | tray-icon + muda |
| 桌面应用 | Tauri v2 |
| 加密 | AES-256-GCM · Ed25519 · SHA-256 |
...
| [第三方声明](../THIRD-PARTY-NOTICES.md) | 784 个依赖的许可证归属 |
